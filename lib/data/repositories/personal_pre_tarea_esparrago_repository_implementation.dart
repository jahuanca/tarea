...
class PersonalPreTareaEsparragoRepositoryImplementation
  ...
  Future<int> create(
    ...
    int idDB= await database.insert(TABLE_NAME_PERSONAL_PRE_TAREA_ESPARRAGO,
      detalle.toDB(),
  ...
  Future<void> delete(String box, int key) async {
    ...
    Database database = await openDatabase(join(await getDatabasesPath(), 'tareo_esparrago.db'));
    await database.delete(TABLE_NAME_PERSONAL_PRE_TAREA_ESPARRAGO,
      where: "id = ?",
      whereArgs: [idSQLite],
    );
  ...
  Future<void> update(
    ...
    Database database = await openDatabase(join(await getDatabasesPath(), 'tareo_esparrago.db'));
    await database.update(TABLE_NAME_PERSONAL_PRE_TAREA_ESPARRAGO,
      detalle.toDB(),
      where: "id = ?",
